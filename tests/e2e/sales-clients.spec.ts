...
test("unauthenticated users are redirected away from sales clients page", async ({ page }) => {
  await page.goto("/protected/sales/clients");
  await expect(page).toHaveURL(/\/auth\/login/);
});

test("sales clients page shows core form elements", async ({ page }) => {
  const email = process.env.E2E_SALES_LOGIN_EMAIL;
  const password = process.env.E2E_SALES_LOGIN_PASSWORD;

  test.skip(
    !email || !password,
    "Set E2E_SALES_LOGIN_EMAIL and E2E_SALES_LOGIN_PASSWORD to validate sales clients UI.",
  );
