...
-- Exact-decimal landed unit cost (migration 0024). raw_cost is the input
-- column Engineering types a clean 2-decimal raw material+labor figure
-- into; the +3% OPEX loading and +1.5% delivery fee (Q = P*1.03, R = Q*1.015)
-- are applied automatically. unit_cost remains a plain column: for new
-- items it's the *display* landed cost (computed app-side by
...
--
-- line_total ROUNDS the landed unit cost to the centavo BEFORE multiplying
-- by quantity (migration 0031) -- superseding migration 0024's original
-- "carry full precision, round once" rule, which mirrored the source Excel
-- template but didn't match how the client's own costing sheet reconciles
-- (Engineering rounds the landed unit cost to 2dp first, then multiplies by
-- quantity). ROUND(numeric, 2) is IMMUTABLE (legal in a generated column
-- expression) and half-up away-from-zero, matching lib/sales/pricing.ts's
-- round2 -- raw_cost is CHECK'd >= 0, so the away-from-zero-vs-half-up
-- distinction never bites. This guarantees unit_cost * quantity === line_total
-- exactly, reconciling with a manual per-unit costing sheet. Falls back to
-- the pre-existing quantity * unit_cost behavior when raw_cost is NULL.
ALTER TABLE public.quotation_items
